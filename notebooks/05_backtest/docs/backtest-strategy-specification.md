...
Normal purchases use completed monthly observations. Daily profit-taking
replaces the current month's observation with today's value, so each stage
uses 119 completed monthly observations plus the current trading-day value.
Both paths retain a 120-observation, current-inclusive, two-stage rolling
transformation.
...
### Daily CAPE construction

For completed Shiller month `m`, recover the CAPE earnings denominator:

```text
E10_m = Shiller_Price_m / Shiller_CAPE_m
```

For every trading day `t` in the following month `m+1`:

```text
Daily_CAPE_t = SPX_Close_t / E10_m
```

The denominator remains fixed inside the month and updates only at the next
month boundary. The current month's Shiller Price and CAPE are never used, as
Shiller Price is a full-month average. Missing prior-month inputs remain
missing and are not forward-filled. The audit output records SPX, Daily CAPE,
the applied E10, its source month, and the source Shiller Price and CAPE.
...
Each tier can execute only once per armed cycle. Crossing multiple tiers in a
single day executes every newly crossed tier in ascending order. After the
expensive-direction Z-score returns to zero sigma or below, rearm all three
tiers. Sell profitable lots first and maintain lot-level cost basis and
realized profit records. Sale proceeds return to cash.
