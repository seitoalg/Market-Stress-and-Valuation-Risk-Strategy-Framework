...
def prepare_vix_observations(
    ...
    """Return independently validated VIX Close and High series.

    A missing or non-positive High must not remove an otherwise valid Close.
    The two fields serve different analyses: Close measures the distribution
    and rolling model, while High classifies stress events.
    """
    missing = [col for col in (high_col, close_col) if col not in raw.columns]
    if missing:
        raise KeyError(f"Missing required columns: {missing}")

    high = pd.to_numeric(raw[high_col], errors="coerce")
    close = pd.to_numeric(raw[close_col], errors="coerce")
    valid_high = high.loc[high.notna() & high.gt(0)].rename("VIX_High")
    valid_close = close.loc[close.notna() & close.gt(0)].rename("VIX_Close")

    diagnostics = pd.Series(
        {
            "High_Missing": int(high.isna().sum()),
            "High_Nonpositive": int(high.notna().mul(high.le(0)).sum()),
            "Close_Missing": int(close.isna().sum()),
            "Close_Nonpositive": int(close.notna().mul(close.le(0)).sum()),
            "Valid_Close_Without_Valid_High": int(
                (~valid_close.index.isin(valid_high.index)).sum()
            ),
        },
        dtype="int64",
    )
    return valid_close, valid_high, diagnostics
...
def compute_vix_risk_frame(
    ...
    result = monthly.copy()
    result["Log_VIX"] = log_transform(values)
    rolling = compute_two_stage_rolling_risk(result["Log_VIX"], window=window)
    result["Log_10Y_Avg"] = rolling["Rolling_Mean"]
    result["Log_Deviation"] = rolling["Deviation"]
